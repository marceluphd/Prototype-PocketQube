...
uint16_t lora_returnbandwidth(uint8_t BWregvalue)
{
  switch (BWregvalue)
  {
    case 0:
...
void lora_Tone(int16_tint ToneFrequency, uint16_t ToneLength, uint8_t TXPower)
{
...
uint8_t lora_Send(uint8_t TXBuffStart, uint8_t TXBuffEnd, char TXPacketType, char TXDestination, char TXSource, uint32_t TXTimeout, uint8_t TXPower, uint8_t StripAddress)
{
